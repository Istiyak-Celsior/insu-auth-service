...
func StartServer(container *configuration.Container) {
	...
	go func() {
		...
		log.Printf("WebSocket endpoint: ws://localhost:%d/chat/ins/%s", container.Config.Server.SocketPort, container.Config.ChatDatabase.SocketRoute)
...
func configureServer(container *configuration.Container, h *hub.Hub) *http.Server {
	...
	router.Use(cors.New(cors.Config{
		AllowOrigins:     []string{"http://localhost:3001", "http://localhost:3000", "https://www.celsior-tech.com"},
	...
	router.GET("/", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{
			"message": "Welcome to Insurance_Chat Application Server!",
		})
	})

	// WebSocket route
	// User connects once with userId, then subscribes to conversations dynamically
	wsRoute := "/ins/" + container.Config.ChatDatabase.SocketRoute
	...
	InqueryRouters(router, container)
